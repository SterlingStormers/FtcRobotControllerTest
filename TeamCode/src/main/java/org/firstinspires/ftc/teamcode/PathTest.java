...
//        panelsTelemetry.debug("desired V", "(" + mpc.desiredVx + ", " + mpc.desiredVy + ", " + mpc.desiredOmega + ")");
    ...
    public int autonomousPathUpdate() {
        ...
        switch (pathState) {
            ...
            case 9:
                if (!follower.isBusy()) {    // ← was !follower.isBusy()
                    telemetry.addData("followPath: ", 2);
                    follower.followPath(paths.Path2);   // ← was follower.followPath(paths.Path2, true)
            ...
            case 10:
                ...
                if (!follower.isBusy() && pathTimer.getElapsedTimeSeconds() >= waitTime) {    // ← was !follower.isBusy()
                    SpindexerLogic1();
                }
                break;
            ...
            case 14:
                drive.shooterMotor.setPower(0);
                if (!follower.isBusy()) {    // ← was !follower.isBusy()
                    follower.followPath(paths.Path3);    // ← was follower.followPath(paths.Path3, true)
